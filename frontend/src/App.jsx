...
export default function App(){
  ...
  async function checkBackend(){
    try{
      const r=await axios.get(`${API}/health`,{timeout:4000});
      if(r.data?.status==="ok"){setConnected(true);loadMenu();}
  ...
  async function handlePreview(){
    ...
    try{
      ...
      fd.append("manual_text",JSON.stringify(manualText));
  ...
  function approveCurrentSlide(){
    const config = {
      ...
      year_label: manualText.year_label||"", footnote: manualText.footnote||"",
    };
    setPresSlides(prev=>{
      const updated=[...prev];
  ...
  return(
    <div className="app-shell">
      ...
      <header className="app-header">
        <div className="header-inner">
          <div className="header-brand">
            ...
            <div>
              <h1 className="brand-title">District Slide Generator</h1>
      ...
      <main className="app-main">
        {connected===false&&(
          <div className="alert alert-error">
            <strong>Backend offline.</strong> Run: <code>python -m uvicorn main:app --reload</code> from <code>backend/</code>, then <button className="retry-btn" onClick={checkBackend}>retry</button>
          </div>
        )}
              ...
              <>
                ...
                <div className="card" style={{padding:"14px 20px"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                    ...
                    <button className="link-btn" style={{color:"#E8192C"}} onClick={()=>{setPresPhase("plan");}}>← Back to plan</button>
                  </div>
                  <div style={{display:"flex",gap:4}}>
                    {presSlides.map((s,i)=>(
                      <div key={i} style={{flex:1,height:6,borderRadius:3,background:s.status==="approved"?"#16A34A":s.status==="skipped"?"#9CA3AF":i===presCurrentIdx?"#00B0F0":"#E5E7EB"}}/>
                    ))}
                  </div>
                  <div style={{display:"flex",gap:16,marginTop:6}}>
                    {presSlides.map((s,i)=>(
                      <div key={i} style={{fontSize:9,color:s.status==="approved"?"#16A34A":s.status==="skipped"?"#9CA3AF":i===presCurrentIdx?"#00B0F0":"#9CA3AF",textAlign:"center",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</div>
                    ))}
                  </div>
                </div>
      ...
      <footer className="app-footer">District Slide Tool · Economic Mobility Center · Internal use only</footer>
    </div>
  );
            ...
            {colDetection&&(
              <div className="mapping-panel">
                <div className={`mp-title ${hardMissing.length?"warn":"ok"}`}>
                  {hardMissing.length?`⚠ ${hardMissing.length} column(s) need selection:`:"✓ Columns ready"}
                </div>
          ...
          <div className="card">
            ...
            <div className="metadata-grid">
              {(()=>{
                ...
                const allFields = [
                  {key:"District",    label:"District / Organization Name",  placeholder:"e.g. Grand Prairie ISD"},
            ...
            {!preview?(
              ...
              <div className="preview-panel">
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  ...
                  <button className="preview-btn" style={{padding:"7px 16px",fontSize:12,display:"flex",alignItems:"center",gap:6}} onClick={fetchSlidePreviewHtml}>
                    🖼 Preview Slide
                  </button>
                </div>
                ...
                <div className="prev-labels">
                  {Object.entries(editLabels).sort(([a],[b])=>a==="Title"?-1:b==="Title"?1:0).map(([key,val])=>(
          ...
          <div className="card generate-card">
            ...
            {generating&&(
              <div className="loading-status">
                <span className="spinner dark" style={{width:14,height:14}}/>
                <span>Building slide with chart and branding — downloading when ready…</span>
              </div>
            )}
